...
import threading
import warnings
from typing import Dict, Set, Optional, Iterable, Mapping, Any, Callable, Union

from src.services.logger import logger
from src.services.threadmanager.callback import Callback
from src.services.threadmanager.config import IDLE, START, STOP, MIN_POOL_MONITOR_INTERVAL, THREAD, \
    DEFAULT_WORKER_COUNT, MIN_THREAD_LAUNCHER_TIMEOUT, FUTURE
from src.services.threadmanager.thread_exceptions import StopNotificationWarning, WaitTimeout
from src.services.threadmanager.thread_statistics import record_statistics
from src.services.threadmanager.thread_utils import get_caller, get_func_name, pluralize, thread_nametag
from src.services.threadmanager.timed_future import TimedFuture, TimedFutureThreadPool
from src.services.threadmanager.timed_thread import TimedThread
...
class ThreadManager(object):
    ...
    def add(self, pool_name: str, func: Callable, args: Iterable = (), kwargs: Optional[Mapping[str, Any]] = None,
            get_ref: bool = False, timeout: Optional[float] = None, tag: str = ""):
        """
    ...
    def add_pool(self, name: str, pool_type: str = THREAD, runtime_alert: float = 0,
                 worker_count: int = DEFAULT_WORKER_COUNT) -> 'ThreadPoolController':
        ...
        with self._rlock:
            ...
            new_pool = ThreadPoolWrapper(self, name, pool_type, runtime_alert, self._pool_idle_event,
                                         worker_count=worker_count, safe=self._safe)
...
class ThreadPool(object):
    ...
    def __init__(
            self, master: 'ThreadPoolWrapper', name: str, max_running: int = DEFAULT_WORKER_COUNT,
            runtime_alert: float = 0.0,
            safe: bool = True
    ):
        self._master = master
    ...
    def submit(self, tag: str, func, *args, **kwargs):
        ...
        new_thread = TimedThread(
            master=self._master, pool=self, tag=tag, target=func, name=thread_name, args=args, kwargs=kwargs,
            safe=self._safe
        )
...
class ThreadPoolWrapper(object):
    ...
    def __init__(self, master: ThreadManager, name: str, pool_type: str, runtime_alert: float,
                 idle_event: threading.Event, worker_count: int = DEFAULT_WORKER_COUNT, safe: bool = True):
        """
        ...
        if pool_type == FUTURE:
            self._pool = TimedFutureThreadPool(self, runtime_alert=runtime_alert, max_workers=worker_count,
        ...
        with self._rlock:
            ...
            if self._statistics_enabled:
                if self.use_tag_in_stats:
                    record_statistics(self._name, thread_name, thread_item.total_runtime())
                else:
                    record_statistics(self._name, thread_item.name, thread_item.total_runtime())
            if self.idle():
...
class ThreadRequest(object):
    ...
    def __init__(self, pool: ThreadPoolWrapper, func: Callable, args: Iterable, kwargs: Mapping[str, Any],
                 get_ref: bool, tag: str):
        self._rlock = threading.RLock()
